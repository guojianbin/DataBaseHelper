...
namespace DbHelper.HelperMini
{
    ...
    public sealed class SqlHelperMini
    {
            ...
            try
            {
                switch (type)
                {
                    case HelperConfigType.appSettings:
                        conn = ConfigurationManager.AppSettings[strConfig];
                        break;
        ...
        /// <returns>查询表的SQL</returns>
        private string GetTable(string tableName, string cellName = null)
        {
            if (string.IsNullOrEmpty(cellName))
            {
                cellName = "*";
            }
            return string.Format("select {0} from {1}", cellName, tableName);
        }

        /// <summary>
        /// 判读传递参数是否为表名
        /// </summary>
        /// <param name="strText">参数</param>
        /// <param name="type">传递类型</param>
        private void IsTable(ref string strText, ref CommandType type)
        {
            if (type == CommandType.TableDirect)
            {
                strText = GetTable(strText);
        ...
        /// <returns></returns>
        public SqlDataReader RunToDataReader(string sql)
        {
        ...
        /// <summary>
        /// 执行存储过程，返回DataReader对象
        /// </summary>
        /// <param name="sql">SQL语句或存储过程名</param>
        /// <param name="type">类型</param>
        /// <param name="pars">参数变量</param>
        /// <returns></returns>
        public SqlDataReader RunToDataReader(string sql, CommandType type, SqlParameter[] pars = null)
        {
            SqlCommand sqlCommand = null;
            if (pars == null)
            {
                sqlCommand = GetCommand(sql, type);
            }
            else
            {
                sqlCommand = GetCommand(sql, pars, type);
            }
